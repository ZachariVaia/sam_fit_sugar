...
# Usage:
#   ./run_sam2.sh <dataset_name-or-subfolder>
# Env:
#   WEB_GUI=1             # web GUI (Gradio)
#   WEB_GUI_MODE=first    # first | all
#   GRADIO_PORT=7860      # port για το Gradio (optional)
#   GUI=1                 # or, native matplotlib GUI
#   GUI_MODE=first        # first | all

set -euo pipefail

# --- args FIRST (πριν από οτιδήποτε χρησιμοποιήσει DATASET_NAME) ---
if [[ $# -lt 1 && -z "${DATASET_NAME:-}" ]]; then
...
SAM2_DOCKER_PATH="${SAM2_DOCKER_PATH:-$REPO_DIR}"

# --- Silence everything to a log (δημιουργείται ΑΦΟΥ ξέρουμε DATASET_NAME) ---
LOGDIR="$SAM2_DOCKER_PATH/logs"
mkdir -p "$LOGDIR"
LOGFILE="$LOGDIR/${DATASET_NAME}_$(date +%Y%m%d_%H%M%S).log"

# Keep the original stdout/stderr (για να τυπώνουμε μόνο σε σφάλμα)
exec 3>&1 4>&2
...
def load_points_for_image(img_path, dataset_root):
    ...
    base = os.path.splitext(os.path.basename(img_path))[0]
    same_dir = os.path.dirname(img_path)
    # Cross-platform relative path σε POSIX μορφή
...
for mpath in mask_paths:
    base = os.path.basename(mpath)
    stem = base[:-9]  # Remove '_mask.png' from the mask filename
    ...
    ipath = candidates[0]
    im = Image.open(ipath).convert("RGBA")  # Open the image in RGBA mode
    mimg = Image.open(mpath).convert("L")   # The mask is in grayscale

    if mimg.size != im.size:
        mimg = mimg.resize(im.size, Image.NEAREST)  # Resize the mask to match the image if necessary

    # Create a new image with a black background
    im_black = im.copy()
    black_pixels = mimg.point(lambda p: 0 if p == 0 else 255)  # Black where the mask is 0

    # Apply the mask to the image with the black background
    r, g, b, a = im.split()
    ...
    # Create the new image with the corresponding pixels
